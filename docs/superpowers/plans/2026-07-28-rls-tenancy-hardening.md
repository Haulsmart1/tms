...
`rls_01` (add column) → **`rls_01b` (reseed: companies, tenants, profiles.tenant_id, admin roles)** → `rls_02` (helpers) → `rls_03` (re-key) → `rls_04` (identity) → `rls_04b` (vehicles) → `rls_05` (revokes) → `rls_06` (lock secrets) → `rls_09` (verify) → then the optional NOT NULL / data cleanup.
...
- **Fifth confirmation review (2026-07-28):** ZERO live security holes; core isolation, escalation guards, and fail-closed helpers all re-verified sound. Five LOW polish items folded in: `rls_01b` scopes the admin promotion to the first profile per company (F-C); `rls_09` fails fast if the staff id is a super_admin and reports INCONCLUSIVE on an empty tenant instead of a false PASS (F-E/F-F); `rls_06` revokes anon SELECT on the admin_read tables and tightens default privileges for future tables (F-A/F-D). A CI check that new public tables have RLS + a policy is the one remaining governance follow-up.
- **Final outlier review (2026-07-28):** core isolation verified clean. FIXED in `rls_06`: `integration_connections` (plaintext OAuth tokens) locked to service-role only (it had been staff-readable via the writes_closed read policy), and the stale `TO public` policy on legacy `memberships` dropped. Owner decisions (also in `rls_06`): `driver_work_rules` -> admin-write (WTD limits are admin-managed), `defect_reports` stays staff-writable (drivers file their own defects); `subscriptions`/`telematics_devices` -> admin-read only. The hardcoded-`2f7cc0dc` app breakage (the app can't reach the reseeded tenants) is the deferred de-hardcode; the app will not work for real users until it lands. Regenerate the schema dump post-apply to confirm the end-state.
