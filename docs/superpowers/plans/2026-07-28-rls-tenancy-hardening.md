...
- **Final outlier review (2026-07-28):** core isolation verified clean. FIXED in `rls_06`: `integration_connections` (plaintext OAuth tokens) locked to service-role only (it had been staff-readable via the writes_closed read policy), and the stale `TO public` policy on legacy `memberships` dropped. OPEN decisions (intent-dependent): should `driver_work_rules`/`defect_reports` be admin-write like the roster, and should `subscriptions`/`telematics_devices` identifiers be admin-read rather than staff-read. The hardcoded-`2f7cc0dc` app breakage (the app can't reach the reseeded tenants) is the deferred de-hardcode; the app will not work for real users until it lands. Regenerate the schema dump post-apply to confirm the end-state.
